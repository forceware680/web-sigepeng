...
function appToDb(tutorial) {
    return {
        ...
        published_at: tutorial.publishedAt || tutorial.createdAt,
...
export async function readSupabaseTutorials(options = {}) {
    ...
    const { data: tutorials, error: tutorialsError } = await query.order('order', { ascending: true });
...
export async function readTutorials(options = {}) {
    if (isSupabaseConfigured()) {
        try {
            return await readSupabaseTutorials(options);
        } catch (error) {
...
export async function getTutorialsByCategory(categoryId, options = {}) {
...
export async function getTutorialsGroupedByCategory(options = {}) {
    const tutorials = await readTutorials(options);
    const grouped = {};
...
export async function getRelatedTutorials(currentId, categoryId, limit = 3) {
    // Only show published related tutorials
    const tutorials = await readTutorials({ publicOnly: true });
